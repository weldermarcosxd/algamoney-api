...
public class AlgaMoneyExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private ResponseEntity<Object> handleDataIntegrityViolationException(DataIntegrityViolationException ex, WebRequest request){
		...
		String stacktrace = ExceptionUtils.getRootCauseMessage(ex);
		List<Error> errorList = Arrays.asList(new Error(userMessage, stacktrace));
		return super.handleExceptionInternal(ex, errorList, new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
